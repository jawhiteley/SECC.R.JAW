...
SECC.full <- SECC                       # make a copy, for further processing (save the original for reference).

## Define Labels
Y.col   <- 'ARA.m'  # Column to use as main response variable.
Y.label <- attr(SECC, "labels")[Y.col]	  # response variable label for this script.
Dataset.list <- c("SECC", "SECCmc")
Dataset.labels <- c( "Patch scale", "Meta-Community scale" )
...
SECC.full <- within( SECC.full, {	
  ## Generic Response Variable (used in remainder of script)
  Y <- as.numeric( get(Y.col) )
  Y[Y < 0] <- 0  # change negative ARA values to 0 - should I wait until after aggregation?
})

## Filter data for analysis.
SECC.full <- SECC.full[SECC.full$Time==levels(SECC.full$Time)[1], ]	# only one time point at a time (not repeated measures)
...
SECCmc <- SECC_aggregate( SECC.full, trt = 'Frag' )

## aggregate 'other' patch Positions
SECCp  <- SECC_aggregate( SECC.full, trt = 'Position' )
...
for ( dataset in dataset.list ){
  assign( dataset,
         within( get(dataset), {
           Y[Y < 0] <- 0         # negative values are problematic for log and other transformations :-(
...
Yr.aov <- aov( Yr.model, data=SECCmc )	# regional effects only.
...
## REGIONAL analyses
	xyplot( Y.use ~ Block | Frag + Chamber, data=SECCmc, 
		pch=21, col="black", bg="grey", cex=0.8,
		main = dataset.labels[2]
	)
par( mfrow=c(2,2), cex=0.8)	# panel of figures: 2 rows & 2 columns
# homogeneity of variances?
with( SECCmc, plot(Y.use ~ Chamber*Frag) )	# fixed effects only, no nesting
# normal distribution?
with(SECCmc, qqnorm(resid(Yp.aov$Within), main="Residuals - Regional" ) )	# are residuals normally distributed?
par( mfrow=c(1,1) )
hist(resid(Yr.aov$"Block:Chamber:Frag"))	# plot residuals
# with(SECCmc, shapiro.test( Y.use ) )	# normality?
...
with( SECCmc, interaction.plot( Frag, Chamber, Y.use, ylab=paste("mean of ", Y.used) ) )

## Planned Multiple Comparisons using Least Significant Differences (LSD) -> comparison intervals for graphical display.
lsd.r <- LSD( Yr.aov$"Block:Chamber:Frag", Yr.model, data=SECCmc, alpha=0.05, mode="pairwise" )	# compute LSDs based on a 5% error rate (alpha), 2-tailed.
lsd.r.FxC <- lsd.r["Chamber:Frag"]
lsd.r <- LSD( Yr.aov$"Block:Chamber", Yr.model, data=SECCmc, alpha=0.05, mode="pairwise" )	# compute LSDs based on a 5% error rate (alpha), 2-tailed.
